...
export async function onRequestDelete(context) {
  ...
  const imageFileName = normalizeImageFileName(params.fileName);
  if (!imageFileName) {
    return errorResponse('VALIDATION_ERROR', '유효하지 않은 이미지 파일명입니다.', 400);
  }

  try {
    const result = await deleteCaseImage(env, contentType, mdFileName, imageFileName);
    if (!result.ok) {
    ...
    return successResponse({
      ...
      imageKeptForOtherCases: result.imageKeptForOtherCases,
      remainingGallery: result.remainingGallery,
    ...
    return errorResponse('SERVER_ERROR', '이미지 삭제에 실패했습니다.', 500);
  }
}
