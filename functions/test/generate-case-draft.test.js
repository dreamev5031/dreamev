...
const sampleDraft = {
  ...
  inspectionResult: '점검 결과 전진 신호 계통의 배선 단선이 확인되었습니다.',
...
test('sanitizeDraftFields rejects template particles', () => {
  assert.throws(
    ...
    (err) => err.code === 'OPENAI_INVALID_RESPONSE',
  );
});
...
test('callOpenAiDraft succeeds with structured output', async () => {
  ...
  assert.equal(result.draft.inspectionResult.length > 0, true);
});
...
test('generate-case-draft handler returns success draft', async () => {
